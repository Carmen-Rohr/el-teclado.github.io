...
const cuandoSeHaceClick = function (evento) {
	...
	if (texto.textContent.length==1 && this.innerText=="T") {
	  	texto.textContent= texto.textContent+this.innerText;
		}
		else if (texto.textContent.length==1 && this.innerText !="T") {
			texto.textContent= texto.textContent+this.innerText;			
			music.play();
		} else if (texto.textContent.length==2 && this.innerText=="E") {
		texto.textContent= texto.textContent+this.innerText;
		}
	    else if (texto.textContent.length==2 && this.innerText !="E") {
	   	    texto.textContent= texto.textContent+this.innerText;			
			music.play();
		} else if (texto.textContent.length==3 && this.innerText=="L") {
			texto.textContent= texto.textContent+this.innerText;
			}
			else if (texto.textContent.length==3 && this.innerText !="L") {
				texto.textContent= texto.textContent+this.innerText;			
		...
		} else if (texto.textContent.length==4 && this.innerText=="E") {
			texto.textContent= texto.textContent+this.innerText;
			const tele = new Audio('./sonidos/tele.ogg');
			tele.play();
			setTimeout(function(){			
				...
				function mostrar() {
					div = document.getElementById('estrella4');
					div.style.display = '';
				};
				function cerrar() {
					div = document.getElementById('estrella3');
					div.style.display = 'none';
				};
				cerrar();
				mostrar();
